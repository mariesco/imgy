...
const fetcher = (url: string) => fetch(url).then((res) => res.url);

type MainViewedImageProps = {
  imageViewed: ImageViewed;
  isLoadingImages: boolean;
}

export const MainViewedImage: FC<MainViewedImageProps>  = ({imageViewed, isLoadingImages}) => {

  const [height] = useState(0)
  const [width, setWidth] = useState(0);
  const { data, error, isLoading } = useSWR(imageViewed.image.url, fetcher)
  const isLoadingView: boolean = isLoadingImages || !data || isLoading;
  ...
  if(isError){
    return (
    <div data-testid="error-on-viewed-image">
      ...
      <div className="flex items-center justify-center h-12 mb-4 rounded bg-gray-800">
        <p className={`${inter.className} text-red-500 font-semibold pr-4`}>
            Error generating the image, we recommend refreshing the page
        </p> 
      </div>
    </div>
    )
  ...
  if(isLoadingView){
    return (
    <div data-testid="loading-viewed-image">
      <div className="flex items-center justify-center mb-4 h-72">
        {renderSpinner()}
      </div>
      <div className="flex items-center justify-center h-12 mb-4 rounded bg-gray-800">
